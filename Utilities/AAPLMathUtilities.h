...
// A uint16_t is 2 bytes which has the same size as a 16-bit float
// (aka as half in MSL syntax)
static inline void storeAsF16(float value, uint16_t *pointer) {
    *(__fp16 *)pointer = value;
}
